...
export default function SingleOrganizedEventToggleListButton({ active, onClick, label, withBadge, badgeCount }: SingleOrganizedEventToggleListButtonProps) {
  return (
    <button
      className={`relative flex items-center gap-6 md:text-2xl font-semibold ${active ? 'bg-slate-500/50' : 'text-stone-600 bg-slate-600/20'}  md:px-10 md:py-4 p-2 rounded-xl border border-gray-400/20 hover:scale-[0.99] cursor-pointer transition-all`}
      onClick={onClick}
    >
      {label}
